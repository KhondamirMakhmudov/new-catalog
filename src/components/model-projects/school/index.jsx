...
const SchoolProject = () => {
  ...
  const totalItems = get(school, "data.count");
  ...
  return (
    <div>
      {isLoadingSchool || isFetchingSchool ? (
        <ContentLoader />
      ) : (
        <motion.table
        ...
        >
          ...
          <tbody>
            {get(school, "data.results", []).map((item, index) => (
              <tr
                key={get(item, "id")}
      ...
      <div className="py-[20px] px-[24px] bg-white rounded-br-[12px] rounded-bl-[12px] flex items-center justify-between">
        <div>
          <p className="text-sm text-[#9392A0]">
            {" "}
            {get(school, "data.count")} tadan 1-{limit} tasi ko&apos;rsatilgan
          </p>
        </div>
