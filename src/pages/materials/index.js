...
const Index = () => {
  ...
  const handleCheckboxChange = (item) => {
    setSelectedItems((prevSelected) => {
      const isSelected = !prevSelected[item.id];
      const updatedSelection = { ...prevSelected };

      if (isSelected) {
        updatedSelection[item.id] = true; // ID ni qo'shish
      } else {
        delete updatedSelection[item.id]; // ID ni olib tashlash
      }

      return updatedSelection;
    });

    // `setSelectedItems` async ishlaydi, shuning uchun keyingi stepda `setTimeout` yoki `useEffect` ishlatish kerak
  };
  ...
  useEffect(() => {
    ...
    if (selectedIds.length > 0) {
      console.log("🚀 Sending request with IDs:", selectedIds);

      fetch("https://backend.mkinfo.uz/fastapi/material_ads/", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Accept: "application/json",
        },
        body: JSON.stringify(selectedIds), // Yoki { ids: selectedIds } qilib ko'ring
      })
        .then(async (response) => {
          if (!response.ok) {
            console.error(
                ...
                <div className="font-gilroy bg-white  border border-[#E0E2F0] rounded-[12px]">
                  <div className="overflow-x-auto">
                    ...
                    >
                      ...
                      <tbody>
                        {get(tableData, "materials", [])?.map((item, index) => (
                          <tr
                            key={get(item, "id")}
                            className="text-sm odd:bg-[#EDF4FC] even:bg-white"
                          >
                  ...
                  <div className="py-[20px] px-[24px] bg-white rounded-br-[12px] rounded-bl-[12px] flex flex-col md:flex-row items-center justify-between">
                    <div>
                      {/* <p className="text-sm text-[#9392A0]">
                        {" "}
                        {get(tableData, "total_items")} tadan 20 tasi
                        ko&apos;rsatilgan
                      </p> */}
                    </div>
                    ...
                    <div>
                      <Pagination
                        pageCount={get(tableData, "total_pages")}
                        page={page}
                        setPage={(prev) => setPage(prev)}
                      />
                    </div>
